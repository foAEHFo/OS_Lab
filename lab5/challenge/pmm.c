    ...
    do
    {
        ...
        if (*ptep & PTE_V)//如果源页表项有效并且存在映射
        {
            ...
            //assert(npage != NULL);
             int ret = 0;
            //实现cow
            /* (4) 在目标页表中建立映射，权限与源页不相同 */
            ret = page_insert(to, page, start, perm&~PTE_W);//目标进程的页表项指向源进程的物理页，且去掉写权限，实现写时复制
             /* (5) 关键：也要修改父进程的页表项，去掉写权限！ */
            if (perm & PTE_W) {  // 如果原来有写权限
                *ptep = pte_create(page2ppn(page), (perm & ~PTE_W));  // 父进程页表项也变为只读
