...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
    ...
    off_t endpos = offset + *alenp, blkoff;// 结束位置，块内偏移
    *alenp = 0;
    ...
    size_t size, alen = 0;
    uint32_t ino;
    uint32_t blkno = offset / SFS_BLKSIZE;          // 开始块号
    // 覆盖区间 [offset, endpos) 所跨越的块数（至少为 1）
    uint32_t nblks = (endpos - 1) / SFS_BLKSIZE - blkno + 1;


  //LAB8:EXERCISE1 你的代码提示：调用 sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock 等来读取文件中的不同类型的块
	/*
	 * (1) 如果 offset 没有与第一个块对齐，从 offset 到第一个块的末尾读/写一些内容
     ...
     * (3) 如果结束位置没有与最后一个块对齐，从开始到最后一个块的 (endpos % SFS_BLKSIZE) 读/写一些内容
	 *       注意：有用的函数：sfs_bmap_load_nolock, sfs_buf_op	
	*/
