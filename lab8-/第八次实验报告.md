...
参考本实验中现有的“文件 = inode + file descriptor + vop_read/vop_write”结构，我倾向于把 pipe 作为一种新的 inode 类型（或设备类 inode），让其复用现有的 `sysfile_read/sysfile_write` 路径。
...
- 你的 `do_execve()`/`load_icode()` 已经通过 `sysfile_open/read/seek` 走通了“fd -> 文件系统 -> vop_read”路径。
...
- 你的 `load_icode()` 使用 `mm_create/setup_pgdir/mm_map/pgdir_alloc_page` 建立新的用户地址空间，并把 ELF 段与 BSS 装入内存。
