...
1. 调度器框架与调度算法分离
实验知识点： sched_class 结构体，包含函数指针实现多种调度算法 对应OS原理： 调度器抽象与调度算法可插拔设计
实验中通过函数指针实现调度算法的可插拔，使得框架与算法解耦。体现了操作系统设计中的抽象思想，将调度策略与调度机制分离

2. 时间片轮转调度算法（Round Robin）
实验知识点： RR_enqueue、RR_dequeue、RR_pick_next、RR_proc_tick 等函数的实现 对应OS原理： 时间片轮转调度算法
每个进程分配固定时间片，时间片用完后让出CPU，进入队列尾部，实验代码具体实现了原理中的时间片轮转概念。

3. Stride调度算法
实验知识点： 使用斜堆(skew_heap)实现的Stride调度算法 对应OS原理： 优先级调度算法
通过步长(stride)值来控制进程的调度顺序，优先级高的进程步长增长慢，被调度频率高，Stride算法是实现优先级调度的一种方法。

4. 运行队列(run_queue)
实验知识点： run_queue 结构体同时支持链表和斜堆两种数据结构 对应OS原理： 就绪队列
实验中为了支持不同调度算法，同时使用了链表（RR算法）和斜堆（Stride算法），都是管理就绪进程的数据结构 

5. 时钟中断与调度触发
实验知识点： proc_tick 函数在时钟中断中被调用，检查是否需要重新调度 对应OS原理： 时钟中断机制、时间片管理
通过时钟中断实现时间片的递减和调度决策，实验实现了原理中提到的抢占式调度机制。

6. 进程状态管理
实验知识点： wakeup_proc 函数将进程加入运行队列 对应OS原理： 进程状态转换
进程从阻塞状态转为就绪状态后，需要加入调度队列等待调度，实现了进程状态转换的具体逻辑

7. 多级反馈队列调度算法
实验知识点： 挑战练习中提到的多级反馈队列设计 对应OS原理： 多级反馈队列调度算法
实验中通过扩展调度框架实现更复杂的调度策略，提示了多级反馈队列调度算法的原理和实现方式。
