...
栈上每个寄存器的保存位置是通过宏定义的偏移量确定的，栈是连续的内存区域，sp 指向trapframe的栈顶。每个寄存器占用固定字节数（REGBYTES，32 位），偏移量就是 寄存器在trapframe中的索引×REGBYTES。
...
* **非法指令异常**
  ...
  最后通过对epc+8来更新异常地址寄存器，因为在64位系统中，64 = 8个字节的二进制位，因此触发异常处的下一条指令应该在tf->epc += 8而不是+4处。
