...
之前我介绍过，SAVE_ALL保持了四个CSR，sstatus用来保存异常发生前的状态，sepc保存异常发生时的指令地址（用于异常返回后继续执行），stval保存访问错误的地址,或是其他额外信息，scause保存异常原因（如中断、页故障等）。
...
但是，stval保存访问错误的地址或其他信息,scause保存异常原因（如中断、页故障等）。这些信息对于返回之前的程序没有什么作用了，我只需要在处理中断异常的函数里使用它们，当处理完以后，它们就没有用了，所以不需要进行恢复。当遇见下一个异常中断时，它们会被新的值写入。
...
1. **RISC-V 中断机制**
    ...
    在 RISC-V 架构中，中断嵌套是指处理器在处理一个中断时，可以中止当前中断服务程序，转而处理优先级更高的新中断。这种机制需要硬件和软件的配合，尤其在 RISC-V 中，默认硬件机制并不直接支持中断嵌套，需要通过软件实现。
    而在本次实验中，stvec是中断向量入口寄存器，scause记录中断 / 异常原因，sepc保存异常发生时的指令地址；OS 原理中，中断系统是 OS 响应外部事件（如设备请求、异常）的核心机制，包含中断触发、处理、返回流程。
2. **中断上下文保存与恢复**

    中断上下文保存是上下文切换的一种特殊场景（中断属于 “抢占式” 上下文切换的触发源之一）实验中通过汇编宏保存 / 恢复所有寄存器（通用寄存器 + CSR 寄存器），确保中断处理后程序能回到原状态；OS 原理中，上下文切换是 OS 在进程 / 线程间切换时，保存当前执行环境、恢复目标环境的过程。实验仅聚焦中断触发的上下文保存，且是 RISC-V 架构下的寄存器级操作。
3. **时钟管理与进程调度**

    实验中通过时钟中断实现 “100 ticks” 计数和定时关机，体现时钟中断作为 OS “心跳” 的作用；OS 原理中，时钟是进程调度、定时任务、时间片轮转的基础。原理涵盖时钟的多维度作用（如调度策略、实时性保障），实验仅体现时钟中断的 “计数 + 触发动作” 功能，更偏向工具性验证。
4. **异常处理**

    中断和异常是处理器指令集架构中非常重要的一部分，中断是指处理器在执行程序指令流的过程中被外部请求打断，而异常则是由于处理器内部事件或程序执行中的事件引起的。实验中捕获并处理非法指令、断点异常，输出异常信息；OS 原理中，异常处理是 OS 对程序错误（如非法指令、页故障）的响应机制，包含错误修复、进程终止等策略。


## 列出你认为OS原理中很重要，但在实验中没有对应上的知识点

1. 进程 / 线程管理：实验未涉及进程创建、销毁、调度（如 PCB 结构、fork 系统调用、时间片轮转调度），仅聚焦中断机制，未体现 OS 的 “多任务” 核心能力。

2. 内存管理：实验未涉及虚拟内存、页表、内存分配与回收（如页故障处理、物理内存管理算法），仅在 “裸机” 层面操作，未体现 OS 对内存的抽象与保护。

3. 文件系统：实验无文件的创建、读写、目录管理等操作，未体现 OS 对 “持久化存储” 的抽象能力。

4. 进程间通信（IPC）：实验未涉及管道、消息队列、共享内存等 IPC 机制，仅为单程序的中断逻辑，未体现多任务间的协作。

5. 设备驱动模型：实验对时钟、控制台的操作较底层（直接调用 SBI 函数），未体现 OS 通用的设备驱动框架（如设备抽象层、驱动注册与调度）。
